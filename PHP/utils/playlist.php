...
function show_tracks_of_favorite($id) {
    ...
    try {
        $sql = 'SELECT morceau.titre, contenu_dans.id, morceau.duree,  from contenu_dans JOIN morceau using (id) where id_playlist = (SELECT id_playlist FROM a_creer WHERE id = :id AND is_favorite)';
        $stmt = $database->prepare($sql);
        $stmt->bindParam(':id', $id);
        $stmt->execute();
...
function show_tracks_of_liste_attente($id) {
    ...
    try {
        $sql = 'SELECT morceau.titre, morceau.id, from contenu_dans JOIN morceau using (id) where id_playlist = (SELECT id_playlist FROM a_creer WHERE id = :id AND is_liste_attente)';
        $stmt = $database->prepare($sql);
        $stmt->bindParam(':id', $id);
        $stmt->execute();
        $tracks = $stmt->fetchAll(PDO::FETCH_ASSOC);
